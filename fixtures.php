...
for($i = 0; $i < 30; $i++) {
  ...
  $createdAt = $faker->dateTimeBetween('-3 years');
    ...
    $query->bindValue(':password', password_hash('secret', PASSWORD_ARGON2I)); // password_hash chiffrement du mdp
...
/**
 * Insertion des données de la table "posts"
 */
...
for($i = 0; $i < 80; $i++) {
    ...
    $query = $db->prepare('INSERT INTO posts (category_id, user_id ,title, content, cover, created_at) VALUES (:category_id, :user_id, :title, :content, :cover, :created_at)');
    $query->bindValue(':category_id', rand(1,10), PDO::PARAM_INT);
    $query->bindValue(':user_id', rand(1,30), PDO::PARAM_INT);
